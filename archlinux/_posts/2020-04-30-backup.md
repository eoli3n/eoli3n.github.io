...
- Check the [archlinux wiki comparison table](https://wiki.archlinux.org/index.php/Synchronization_and_backup_programs#Single_machine)
- Remove lines with red cells

That's how you met a great tool, [BorgBackup](https://borgbackup.readthedocs.io/en/stable/).  
Not many tools let you use those features together :
- Encryption
- Data deduplication
- Compression
...
Let's script a bit to use borg on multiple hosts and centralise backups.  
Store your backups on a distant host, borg uses SSH.  
Use RAID to protect from storage failure
...
Borg gets the passphrase required for encryption from env var ``BORG_PASSPHRASE`` to create a backup from host or query, list, mount them on server.  
...
The [prune subcommand](https://borgbackup.readthedocs.io/en/stable/usage/prune.html) purge backups and keep number of backups you set.

Use ``cron`` or a ``systemd-timer`` to automate your backups.  
That's all you need !

Lets test backups on the server.  
``neon`` is my backup server and ``cesium`` is a server I backup.

```
# Init env key
/mnt/sdb/backups root@neon
